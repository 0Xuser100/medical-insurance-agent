...
class ValidationLineItem(BaseModel):
    ...
    status: str = Field(description="Validation status: APPROVED, FLAGGED, or PENDING_REVIEW")
...
class LangChainValidationService:
    ...
    def _build_prompt_template(self) -> PromptTemplate:
        ...
        return PromptTemplate(
            ...
            input_variables=["ocr_data", "diagnosis_mappings", "medication_limit", "min_duration_days", "job_id"],
        )
    ...
    async def _invoke_structured_llm(self, prompt: str) -> ValidationOutputSchema:
        ...
        return await asyncio.to_thread(self.structured_llm.invoke, prompt)
    ...
    def _build_response(
        ...
        for item in result.line_items:
            line_items.append(LineItem(
                ...
                status=self._safe_enum_parse(ItemStatus, item.status, ItemStatus.PENDING_REVIEW),
